function initCommento(global, document) {
  "use strict";
  ...
  function selfLoad(commenter, email) {
    ...
    profileEditButton.innerText = "编辑个人信息";
  ...
  global.popupRender = function (id) {
    ...
    loginLink.innerText = "没有账号？注册一个";
    emailSubtitle.innerText = "邮箱登录";
    emailButton.innerText = "继续";
  ...
  global.popupSwitch = function (id) {
    ...
    emailSubtitle.innerText = "创建一个账号";
    popupBoxType = "signup";
  ...
  global.passwordAsk = function (id) {
    ...
    for (var i = 0; i < order.length; i++) {
      ...
      if (order[i] === "password") {
        ...
        if (popupBoxType === 'signup') {
          fieldButton.innerText = '注册';
        } else if (popupBoxType === 'login') {
          fieldButton.innerText = '登录';
        }
...
initCommento(window.commento, document);
